...
try:
    ...
    class _LuccaSession(requests.Session):
        ...
        def __init__(
            ...
            token: Optional[str],
            *,
            on_refresh: Callable[["_LuccaSession"], None],
        ) -> None:
            super().__init__()
        ...
        def get(self, url: str, **kwargs: Any) -> requests.Response:
            ...
            try:
                ...
                LoginHTMLParser("identity/login", self).get_token(
                    input("Lucca login:"), getpass.getpass()
                )
...
class _ScriptError(ValueError):
    ...
    def __hash__(self) -> int:
        return sum(
            (ord(char) - 64) * 53**i for i, char in enumerate(self._stable_message)
        )
...
def _xtrace(command: Sequence[str]) -> None:
    ...
    sys.stderr.write(
        f"{_XTRACE_PREFIX[0]} "
        + " ".join(f"'{word}'" if _IFS_REGEX.search(word) else word for word in command)
        + "\n"
    )
...
class LoginHTMLParser(html_parser.HTMLParser):
    ...
    def get_token(self, username: str, password: str) -> str:
        ...
        if not self._form:
            raise ValueError(
                f"The parsed HTML at {self._login_url} didn't contain a useful form"
            )
        response = self._session.post(
            "https://bayesimpact.ilucca.net/identity/login",
            data=dict(
                self._form, UserName=username, Password=password, IsPersistent=True
            ),
        )
        response.raise_for_status()
...
class _GitConfig:
    ...
    def recent_reviewers(self, reviewers: list[str]) -> None:
        ...
        self.set_config(
            "review.recent", ",".join(r for r in reviewers if r), is_global=True
        )
    ...
    def lucca_session(self, value: "_LuccaSession") -> None:
        ...
        self.set_config(
            "review.lucca.token", value.typed_cookies.get("authToken"), is_global=True
        )
...
def _get_default() -> str:
    try:
        return _run_git(["rev-parse", "--abbrev-ref", f"{_REMOTE_REPO}/HEAD"]).split(
            "/"
        )[1]
...
def _create_branch_for_review(merge_base: str, username: str) -> Optional[str]:
    ...
    branch = "-".join(
        word.lower()
        for word in _WORD_REGEX.findall(_cleanup_branch_name(title).replace("_", "-"))[
            :2
        ]
    )
    suffixes = {
...
def _get_best_base_branch(
    branch: str, remote: Optional[str], default: str
) -> Optional[str]:
...
def _make_pr_message(refs: _References, reviewers: list[str]) -> str:
    ...
    message = _run_git(
        ["log", "--format=%B", f"{_REMOTE_REPO}/{refs.base}..{refs.branch}"]
    )
...
def _run_git_review_hook(refs: _References, reviewers: list[str]) -> str:
    ...
    if not os.access(hook_script, os.X_OK):
        if path.exists(hook_script):
            logging.warning(
                "The git review hook exists but is not executable. Ignoring."
            )
        return ""
    _xtrace([hook_script])
    try:
...
class _RemoteGitPlatform:
    ...
    def from_url(remote_url: str) -> "_RemoteGitPlatform":
        ...
        raise NotImplementedError(
            f"Review platform not recognized. Remote URL is {remote_url}"
        )
    ...
    def request_review(
        self, refs: _References, reviewers: list[str], is_auto_assigned: bool = False
    ) -> None:
        ...
        if is_auto_assigned and review_id:
            try:
                ...
                logging.warning(
                    "Unable to show to the reviewer they were auto-assigned.",
                    exc_info=error,
                )
    ...
    def _request_review(
        self, refs: _References, reviewers: list[str], message: Optional[str]
    ) -> Optional[str]:
    ...
    def _get_review_number(
        self, branch: str, base: Optional[str] = None
    ) -> Optional[str]:
...
class _GitlabPlatform(_RemoteGitPlatform):
    ...
    def _get_reviewers(self, reviewers: list[str]) -> list[int]:
        return [
            user.id for r in reviewers for user in self.client.users.list(username=r)
        ]

    def _get_merge_request(
        self, branch: str, base: Optional[str]
    ) -> Optional["gitlab.MergeRequest"]:
        return next(
            (
                mr
                for mr in self.project.merge_request.list()
                if mr.source_branch == branch
                if not base or mr.target_branch == base
            ),
            None,
        )
    ...
    def __init__(
        ...
        try:
            ...
            raise _ScriptError(
                ...
                "Please install it with ~/.bayes-developer-setup/install.sh"
            ) from error
...
def _get_platform() -> _RemoteGitPlatform:
    ...
    return _RemoteGitPlatform.from_url(
        _GIT_CONFIG.get_config(f"remote.{_REMOTE_REPO}.url")
    )
...
def _can_review(potential: str, absentee_emails: list[str]) -> bool:
    ...
    selected = (
        ...
        echo "$REPLY"
        """,
                env={"PS3": f"Which is {potential}'s email?"},
                shell=True,
                text=True,
            ).strip()
        )
...
def _get_auto_reviewer() -> Optional[str]:
    ...
    prioritized_reviewers = [
        r
        for r in list(dict.fromkeys(_GIT_CONFIG.recent_reviewers + list(all_engineers)))
        if r in all_engineers
    ][::-1]
...
def _browse_to(branch: str) -> None:
    real_branch = (
        _get_existing_remote() or _get_head() if branch == _BROWSE_CURRENT else branch
    )
...
def main(string_args: Optional[list[str]] = None) -> None:
    ...
    browse_action = parser.add_argument(
        ...
        nargs="?",
        const=_BROWSE_CURRENT,
    )
    if argcomplete:
        setattr(
            reviewer_action,
            "completer",
            lambda **kw: _get_platform().get_available_reviewers(),
        )
        setattr(force_action, "completer", argcomplete.SuppressCompleter())
        setattr(
            browse_action,
            "completer",
            lambda **kw: _get_platform().get_available_reviews(),
        )

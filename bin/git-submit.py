...
def _xtrace(
    command: Sequence[str], *, prefix_cache: list[str] = _XTRACE_PREFIX
) -> None:
    if not prefix_cache:
        return
    sys.stderr.write(
...
def _get_remote_head(base_remote: str) -> str:
    try:
        return _run("git", "rev-parse", "--abbrev-ref", f"{base_remote}/HEAD").split(
            "/"
        )[1]
    except subprocess.CalledProcessError:
        pass
...
def _get_default_branch() -> _Branch:
    ...
    local = next(
        (
            ...
            if local_and_remote[1] == full_remote_head
        ),
        None,
    )
    if not local:
        logging.warning(
            '"%s" is not checked out locally. checking it as "%s".',
            full_remote_head,
            remote_head,
        )
...
def _show_available_branches(
    default_branch: str, remote_prefix: Optional[str] = None
) -> None:
...
def _get_branch(branch: str, default: _Branch, prefix: Optional[str]) -> _Branch:
    ...
    try:
        ...
        if prefix:
            ...
            remote = ""
    merge = remote and _run("git", "config", f"branch.{branch}.merge").removeprefix(
        "refs/heads/"
    )
...
def enable_auto_merge(pr_node_id: str) -> bool:
    ...
    if not mutation_answer["data"]["enablePullRequestAutoMerge"]["pullRequest"][
        "autoMergeRequest"
    ]["enabledAt"]:
        return False
...
def disable_auto_merge(pr_node_id: str) -> bool:
    ...
    if not mutation_answer["data"]["disablePullRequestAutoMerge"]["pullRequest"][
        ...
        return False
    all_comments = _graphql(
        _QUERY_GET_PR_COMMENTS, pullRequestId=pr_node_id, cache=_ONE_MINUTE
    )
...
def get_pr_info(
    branch: str, should_auto_merge: bool = bool(_GIT_SUBMIT_AUTO_MERGE)
) -> Optional[_PrInfos]:
    ...
    can_auto_merge = (
        raw_pr_infos["mergeable"] == "MERGEABLE"
        or raw_pr_infos["viewerCanEnableAutoMerge"]
        or raw_pr_infos["viewerCanDisableAutoMerge"]
    )
...
def _check_clean_state(
    branch: str, default_branch: str, remote_prefix: Optional[str]
) -> None:
    if branch == default_branch:
...
def _should_auto_merge(
    branch: str, should_force: bool, pr_infos: Optional[_PrInfos]
) -> bool:
...
def abort_submit(branch: str, pr_infos: Optional[_PrInfos]) -> None:
    ...
    if not pr_infos:
        logging.warning(
            "Unable to cancel auto-merge (you're not using hub for this project)."
        )
        return
    if not pr_infos.auto_merge.is_enabled:
        logging.info(
            "Auto-merge is not enabled for this pull-request yet. Nothing to abort."
        )
        return
...
def _rebase(
    ...
    *,
    ...
    should_abort_on_conflict: bool = True,
) -> None:
...
def _push_to_remote(
    *, branch: _Branch, default: Optional[_Branch] = None, silently: bool = False
) -> None:
    if default:
...
def _merge_now_or_later(pr_infos: _PrInfos, should_auto_merge: bool, sha1: str) -> bool:
    ...
    if not should_auto_merge:
        ...
        _run_github(
            ...
            "-X",
            "PUT",
            f"/repos/{{owner}}/{{repo}}/pulls/{pr_infos.number}/merge",
            "-F",
            "merge_method=squash",
            "-F",
            f"sha={sha1}",
...
def _user_completer(
    *, prefix: str, parsed_args: _Arguments, **unused_kwargs: Any
) -> set[str]:
    args = _prepare_args(parsed_args)
...
def main() -> None:
    ...
    try:
        should_clean_remote = _merge_now_or_later(
            pr_infos, should_auto_merge, branch.initial
        )
    except subprocess.CalledProcessError:
        abort(default, branch)
    _run("git", "checkout", default.local)
